...
const Register = () => {
    ...
    const saveRegistrationData = async (screenshotUrl, ticketNumber) => {
        const userId = uuidv4();
        await setDoc(doc(db, "bootcamp", userId), {
            ...formData,
            ...
            uploadTask.on(
                ...
                async () => {
                    ...
                    setFormData({
                        ...
                        semester: "",
                        transactionId: "",
                        workshopTopic: "", // Add workshop data here
                        workshopDetails: "",
                        paymentScreenshot: null,
                        referralCode: "",
                    });
    ...
    const handleSubmitPartOne = (e) => {
        e.preventDefault();
        if (validatePartOne()) setPartThree(true);
    };

    const handleSubmitPartThree = (e) => {
        e.preventDefault();
        setPartThree(false);
        setPartTwo(true);
    };
        ...
        <div className="register-form h-fit p-4 pt-0 w-full">
            {!partTwo && !partThree ? (
                ...
                <PartThreeForm
                    formData={formData}
                    handleChange={handleChange}
                    handleSubmit={handleSubmitPartThree}
                />
            ) : (
                <PartTwoForm
                    formData={formData}
...
const PartThreeForm = ({ formData, handleChange, handleSubmit }) => (
    <form onSubmit={handleSubmit} className="w-full flex flex-col   text-white grid-cols-1 gap-3 px-5 md:px-0">
